...
async function getEpisodes4(anilistId, ctx = {}) {
  const anizip = ctx.anizip ?? await getJSON(`${ANIZIP3}?anilist_id=${anilistId}`).catch(() => null);
  const media = ctx.media ?? await getMedia(anilistId).catch(() => null);
  const malId = ctx.media?.idMal ?? media?.idMal ?? anizip?.mappings?.mal_id ?? null;
  const enTitle = anizip?.titles?.en
    ?? ctx.media?.title?.english
    ?? media?.title?.english
    ?? ctx.media?.title?.romaji
    ?? media?.title?.romaji
    ?? (anizip?.titles ? Object.values(anizip.titles)[0] : "")
    ?? "";
    ...
    if (!anizipEps.length) {
      if (media) {
        ...
        const totalEps = media.episodes 
          ?? (media.nextAiringEpisode?.episode ? media.nextAiringEpisode.episode - 1 : 1);
        ...
        return { meta: { malId: null }, episodes: { softsub: sub, dub } };
      }
      throw new Error("Could not find MAL ID and no AniZip episodes available");
    }
  ...
  const allEps = ctx.jikanEps ?? await (async () => {
    const first = await getJSON(`${JIKAN4}/anime/${malId}/episodes?page=1`).catch(() => null);
    if (!first) return [];
    ...
    if (lastPage > 1) {
      const rest = await Promise.all(
        Array.from(
          { length: lastPage - 1 },
          (_, i) => getJSON(`${JIKAN4}/anime/${malId}/episodes?page=${i + 2}`).catch(() => null)
        )
      ).catch(() => []);
      for (const r of rest) {
        if (r && r.data) eps = eps.concat(r.data);
      }
    }
    return eps;
  })();

  if (!allEps.length && media) {
    const totalEps = media.episodes 
      ?? (media.nextAiringEpisode?.episode ? media.nextAiringEpisode.episode - 1 : 1);
    for (let epNum = 1; epNum <= totalEps; epNum++) {
      allEps.push({ mal_id: epNum, title: `Episode ${epNum}`, filler: false, aired: null });
    }
  }
